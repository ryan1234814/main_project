/* ============================================================================
 * ai-compiler.c — minimal AI-dialect compiler -> RISC-V RV64IMAFD assembly
 * ============================================================================
 ...
 *   -O0    : lowered to plain RV64IMAFD scalar loops (flw/fadd.s/fsw, fmadd.s)
 ...
 * Only `x`-separated tokens that are entirely digits count as dimensions, so
 * the element-type token ("f32", "i32", ...) is never mistaken for a dim.  */
static int parse_dims(const char *from, int *dims) {
    const char *ty = strstr(from, "tensor<");
    if (!ty) return 0;
    ty += 7;
    const char *end = strchr(ty, '>');
    if (!end) end = ty + strlen(ty);
    int nd = 0;
    const char *p = ty;
    while (p < end && nd < 2) {
        const char *tok = p, *q = p;
        while (q < end && *q != 'x') q++;
        int isnum = (q > tok);
        for (const char *c = tok; c < q; c++)
            if (!isdigit((unsigned char)*c)) { isnum = 0; break; }
        if (isnum) dims[nd++] = (int)strtol(tok, NULL, 10);
        p = (q < end && *q == 'x') ? q + 1 : end;
    }
    return nd;
}
...
int main(int argc, char **argv) {
    ...
    emit("# Generated by ai-compiler from %s (%s lowering)",
         inpath, emit_hw ? "AISS custom-0 hardware" : "RV64IMAFD software");
    emit("        .option norvc");
    emit("        .text");
    emit("        .align  2");
    ...
    printf("ai-compiler: %s -> %s (%s lowering)\n",
           inpath, outpath, emit_hw ? "AISS hardware" : "RV64IMAFD software");
    return 0;
}
